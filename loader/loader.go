...
func DumpTradeCache(trade *db.TradeHistoryTmp) {
	trade.CreateTradeHistory(trade)
}

func LoadDataBase() {

	history := db.TradeHistoryTmp{}
	...
	var insInfos []db.InscriptionInfoHistory
	ins := db.InscriptionInfoHistory{}
	...
	handlers.GetLogger().Info("InscriptionInfoHistory load succees")
...
func DumpTickerInfoToDB(
	...
	for _, ticker := range allTickers {
		...
		inscriptionInfo := db.InscriptionInfoHistory{
			Ticks:   info.Tick,
...
func DumpBlockNumber() {
	...
	blocnscan := db.HistoryBlockScan{}
	blocnscan.Id = 1
	blocnscan.BlockNumber = int64(handlers.BlockNumber)
	blocnscan.Number = int64(handlers.InscriptionNumber)
	blocnscan.Gas = handlers.Gas
	blocnscan.UptadeBlockNumber(handlers.BlockNumber, handlers.InscriptionNumber, handlers.Gas)
}
